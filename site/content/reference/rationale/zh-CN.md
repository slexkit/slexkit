...
SlexKit 用于聊天消息、文档、agent 输出面板和工具仪表盘中的小型交互 UI。它不提供 routing、data layers、build systems，也不是完整 application framework。

AI-generated UI 的输入格式需要：

- 足够短，适合 token-by-token streaming。
- 可嵌入 Markdown。
- 运行不要求 build step。
- 不绑定宿主，可在 React、Svelte、Obsidian 和 vanilla HTML 中工作。
...
模型可以一次性输出这种结构。它不需要项目结构、module imports、build config 或 framework boilerplate。同一份内容可以放在 Markdown fence、Streamdown renderer、Obsidian adapter 或自定义 runtime host 中运行。
...
SlexKit v0 不是纯 JSON protocol。`$` read-pipes 和 `on*` write-pipes 允许 JavaScript expressions 与 statements：
...
代价是必须有明确 trust boundary。Trusted 内容可以在 host realm 中低成本执行；untrusted 内容必须进入 secure runtime，也就是 sandbox iframe、opaque origin 和由 policy 控制的能力。

SlexKit 的安全选择不是禁用 expressions，而是要求宿主选择 expressions 运行在哪个信任上下文里。
...
````md
```slex
{ namespace: "status", layout: { "badge:state": { label: "3/4 完成", tone: "info" } } }
```
...
大多数 AI output 是展示型内容：status cards、progress indicators、metrics、dashboards。这些走 `slex` fences 或 `mount()`。

ToolHost 只用于必须向宿主返回结构化用户输入的 UI：confirmations、selections、forms。它把 templates 编译为标准 Slex source，并通过 `submit` component 返回结果。

这个规则避免普通 display UI 被误包装成 function call。

## Trusted + secure 双运行时

### Trusted runtime

适用于应用生成内容、仓库维护的 Slex source 或已审查 snippets。集成成本最低，Slex source 直接在宿主页执行。
...
宿主为每次 mount 选择信任上下文；同一 Slex syntax 可在两种模式下运行。
