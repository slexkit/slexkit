  ...
  layout: {
    "card:buck": {
      ...
      "grid:params": {
        ...
        "column:fswField": { "input:fsw": { label: "开关频率 fsw", "$value": "g.fsw", type: "number", unit: "kHz", onchange: "g.fsw = Number($event || 0)" }, "slider:fsw": { label: "fsw", "$value": "g.fsw", min: 50, max: 2000, step: 50, unit: "kHz", onchange: "g.fsw = Number($event)" } },
        "column:ripField": { "input:rippleRatio": { label: "纹波系数 r", "$value": "g.rippleRatio", type: "number", onchange: "g.rippleRatio = Number($event || 0)" }, "slider:rippleRatio": { label: "r", "$value": "g.rippleRatio", min: 0.1, max: 0.5, step: 0.05, onchange: "g.rippleRatio = Number($event)" } }
      },
