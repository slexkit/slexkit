...
Secure runtime 沙盒内提供几类网络任务：读取 posts、查看详情、拉取评论、按用户过滤，或向 JSONPlaceholder 发起演示 POST。请求不会从 iframe 直接出网，而是通过 `api.fetch()` 交给宿主，由宿主按 allowlist、方法、超时、请求体大小和响应类型决定是否代理。
