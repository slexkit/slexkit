...
components: card, input, slider, stat, table, callout, formula, badge, grid, column
difficulty: 入门
runtime: trusted
featured: true
...
项目经理经常需要估算：这个项目要多少人、做多久、花多少钱。这个计算器输入团队规模、开发周期、人员成本，实时算出总成本和明细。

## 核心公式

$$人力成本 = 团队规模 \times 月薪 \times 周期$$

$$设备成本 = 团队规模 \times 5000$$

$$办公成本 = 团队规模 \times 2000 \times 周期$$

$$风险缓冲 = (人力 + 设备 + 办公) \times 15\%$$
  ...
  layout: {
    "card:estimator": {
      ...
      "grid:params": {
        ...
        "column:period": {
          "input:months": { label: "开发周期", "$value": "g.months", type: "number", unit: "月", onchange: "g.months = Number($event || 0)" },
      ...
      "grid:results": {
        columns: 1, mdColumns: 4,
        "stat:team": { label: "团队规模", "$value": "g.teamSize()", unit: "人" },
    ...
    "card:breakdown": {
      ...
      "table:detail": {
        ...
        rows: [
          ...
          ["设备成本", "g.teamSize() + '人 × 5000元'", "g.equipmentCost().toFixed(0)"],
          ["办公成本", "g.teamSize() + '人 × 2000元 × ' + g.months + '月'", "g.officeCost().toFixed(0)"],
          ["风险缓冲（15%）", "g.subtotal().toFixed(0) + ' × 15%'", "g.riskBuffer().toFixed(0)"],
          ["总计", "", "g.totalCost().toFixed(0)"]
        ]
      },
      "callout:tip": {
        tone: "info",
        "$text": "'当前配置：' + g.teamSize() + '人团队，' + g.months + '个月，人均月薪' + g.salary + '元'"
      }
    }
  }
}
...
| 团队规模 | 开发周期 | 人均月薪 | 总成本 |
|----------|----------|----------|--------|
| 3人 | 3个月 | 15,000 | 196,650 |
| 5人 | 6个月 | 15,000 | 655,500 |
| 8人 | 9个月 | 20,000 | 1,989,000 |
| 10人 | 12个月 | 25,000 | 4,140,000 |

**工程笔记**：

- 风险缓冲 15% 是经验值，复杂项目可调高到 20-25%
- 设备成本按一次性 5000 元/人估算（电脑、显示器等）
- 办公成本按 2000 元/月/人估算（工位、水电、差旅等）
- 不包含服务器、云服务、第三方 API 等运营成本
