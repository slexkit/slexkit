...
当宿主不能完全控制 Slex source 时，应使用安全模式（`secure`）：未经审查的用户输入、第三方 Markdown、agent 直接输出，或作者不明确的共享文档。

安全模式需要同时配置部署和 policy。威胁模型、`HostRuntimePolicy`、sandbox 属性、bridge 消息和 fail-closed 行为见 [安全运行时契约](/docs/reference/security)。
...
Markdown 宿主优先使用 `createSlexKitMarkdownRuntimeHost`。它统一处理 artifact 作用域、state-only fence、block 清理和 secure frame 挂载。
...
- 除非有明确产品功能需要，否则保持 network 禁用。
- 如果开启 network，只允许必要的 method、origin、header、body 大小、response 大小和 content type（内容类型）。
...
- 只检测显式 `slex` fence
- 公开 `slexkit.runtime.js` ES module，并配置 CORS 与 JavaScript content type（内容类型）
- Host policy 默认拒绝
- Block 移除和 artifact 销毁时清理运行时
- 每个交互 fence 后保留可读 fallback
- policy、bridge、CSP 和 sandbox 细节链接到参考契约
