...
status: ready
order: 40
summary: "面向不可信或 agent 生成 Slex source 的模式选择与接入指南。"
slexkitRenderMode: component
---

# 安全运行时接入

当宿主不能完全控制 Slex source 时，应使用 secure mode：未经审查的用户输入、第三方 Markdown、agent 直接输出，或作者边界不清楚的共享文档。

Secure mode 是一项部署和 policy 选择。完整威胁模型、`HostRuntimePolicy`、sandbox 属性、bridge 消息和 fail-closed 行为，以 [Security Runtime Contract](/docs/reference/security) 为事实源。

## 模式选择

| Source | 推荐模式 | 说明 |
|---|---|---|
| 应用代码生成的 source | trusted | 开销最低，source 属于应用边界。 |
| 仓库示例或已审查片段 | trusted | 示例应保持显式和版本化。 |
| 本地 Obsidian vault 笔记 | trusted readonly | Obsidian adapter 不是 sandbox 边界。 |
...
Markdown 宿主优先使用 `createSlexKitMarkdownRuntimeHost`。它统一处理 artifact 作用域、state-only fence、block cleanup 和 secure frame 挂载。
...
- 除非有明确产品功能需要，否则保持 network disabled。
- 如果开启 network，只允许必要 method、origin、header、body 大小、response 大小和 content type。
- 除非 Slex source 需要，否则保持 timer、animation 和 canvas disabled。
...
精确 policy 字段和 adapter hooks 以 [Security Runtime Contract](/docs/reference/security) 为准。
...
- policy、bridge、CSP 和 sandbox 细节链接到 reference contract
